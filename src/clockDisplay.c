...
void placeTensUnits(int nValue, utin8_t locX, uint8_t locY, uint32_t nFaceColor);
void placeBit(uint8_t bValue, utin8_t locX, uint8_t locY, uint32_t nFaceColor);
void placeVertBar(utin8_t locX, uint8_t locY);
...
void *threadDigitalClock(void *argp)
{
    ...
    setCurrentBufferFillLEDColor(0x000000);
...
void showCurrBinaryFace(void)
{
    ...
    updateBinaryFace(TU_SECONDS, local->tm_sec);
    updateBinaryFace(TU_MINUTES, local->tm_min);
    updateBinaryFace(TU_HOURS, local->tm_hour);
...
void updateBinaryFace(eTimeUnits tmUnits, int tmValue, uint32_t nFaceColor)
{
    switch(tmUnits) {
        case TU_SECONDS:
            placeTensUnits(tmValue, locTable[OI_SecTens].X, locTable[OI_SecTens].Y);
            break;
        case TU_MINUTES:
            placeTensUnits(tmValue, locTable[OI_MinTens].X, locTable[OI_MinTens].Y);
            break;
        case TU_HOURS:
            placeTensUnits(tmValue, locTable[OI_HrTens].X, locTable[OI_HrTens].Y);
            break;
        default:
            break;
    }
...
void placeTensUnits(int nValue, utin8_t locX, uint8_t locY, uint32_t nFaceColor)
{
    ...
    placeBit(bit7, locX, locY)
    uint8_t bit6 = (nValue & 0x0040) > 0;
    placeBit(bit6, locX, locY+4)
    uint8_t bit5 = (nValue & 0x0020) > 0;
    placeBit(bit5, locX, locY+8)
    uint8_t bit4 = (nValue & 0x0010) > 0;
    placeBit(bit4, locX, locY+12)
    uint8_t bit3 = (nValue & 0x0008) > 0;
    placeBit(bit3, locX+4, locY)
    uint8_t bit2 = (nValue & 0x0004) > 0;
    placeBit(bit2, locX+4, locY+4)
    uint8_t bit1 = (nValue & 0x0002) > 0;
    placeBit(bit1, locX+4, locY+8)
    uint8_t bit0 = (nValue & 0x0001) > 0;
    placeBit(bit0, locX+4, locY+12)
}
...
void placeBit(uint8_t bValue, utin8_t locX, uint8_t locY, uint32_t nFaceColor)
{
    ...
    setCurrentBufferLEDColor(s_nClockBufferNumber, nColor, locX, locY);
    setCurrentBufferLEDColor(s_nClockBufferNumber, nColor, locX+1, locY);
    setCurrentBufferLEDColor(s_nClockBufferNumber, nColor, locX, locY+1);
    setCurrentBufferLEDColor(s_nClockBufferNumber, nColor, locX+1, locY+1);
}
